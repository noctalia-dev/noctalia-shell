                  ...
                  Repeater {
                    ...
                    delegate: Item {
                      ...
                      IconImage {
                        ...
                        property string currentAppId: modelData?.appId || ""
                        ...
                        Component.onCompleted: {
                          if (currentAppId) {
                            const iconName = root.getIconForApp(currentAppId);
                        ...
                        onCurrentAppIdChanged: {
                          if (currentAppId) {
                            // Clear icon immediately to prevent showing stale icons during resolver restart
                            appIconImage.source = "";
                            const iconName = root.getIconForApp(currentAppId);
                            // Resolve with fallback when app ID changes
                            AppSearch.resolveIconWithFallback(iconName, function(path) {
                              if (path && path.length > 0) {
                                appIconImage.source = path;
                              }
                        ...
                        Connections {
                          ...
                          function onReadyChanged() {
                            if (IconResolver.ready && appIconImage.currentAppId && (appIconImage.source === "" || !appIconImage.source)) {
                              // If icon is still empty when resolver becomes ready, try again
                              const iconName = root.getIconForApp(appIconImage.currentAppId);
                              AppSearch.resolveIconWithFallback(iconName, function(path) {
                                if (path && path.length > 0) {
                                  appIconImage.source = path;
                                }
                          ...
                          function onResolverRestarted() {
                            ...
                            if (appIconImage.currentAppId) {
                              appIconImage.source = "";
                              const iconName = root.getIconForApp(appIconImage.currentAppId);
                              
                              // If resolver is restarting, wait for it to be ready before resolving
                              // This prevents mismatched icons when switching apps during theme changes
