...
Rectangle {
    ...
    function updateResults() {
        ...
        // Find first selectable item to default index
        var initialIdx = 0;
        while (initialIdx < results.length && results[initialIdx].isHeader) {
            initialIdx++;
        }
        selectedIndex = initialIdx < results.length ? initialIdx : 0;
    }
    ...
    function selectNext() {
        if (results.length > 0 && selectedIndex < results.length - 1) {
            ...
            while (nextIdx < results.length && results[nextIdx].isHeader) {
                nextIdx++;
            }
    ...
    function selectPrevious() {
        if (results.length > 0 && selectedIndex > 0) {
            ...
            while (prevIdx >= 0 && results[prevIdx].isHeader) {
                prevIdx--;
            }
            if (prevIdx >= 0) {
    ...
    function selectNextWrapped() {
        if (results.length > 0) {
            if (allowWrapNavigation) {
                var nextIdx = (selectedIndex + 1) % results.length;
                var scanned = 0;
                while (scanned < results.length && results[nextIdx].isHeader) {
    ...
    function selectPreviousWrapped() {
        if (results.length > 0) {
            if (allowWrapNavigation) {
                var prevIdx = (((selectedIndex - 1) % results.length) + results.length) % results.length;
                var scanned = 0;
                while (scanned < results.length && results[prevIdx].isHeader) {
                    prevIdx = (((prevIdx - 1) % results.length) + results.length) % results.length;
                    scanned++;
                }
                selectedIndex = prevIdx;
            } else {
    ...
    function selectFirst() {
        if (results.length === 0)
            return;
        var idx = 0;
        while (idx < results.length && results[idx].isHeader) {
            idx++;
        }
        selectedIndex = idx < results.length ? idx : 0;
    }
    ...
    function selectLast() {
        ...
        while (idx >= 0 && results[idx].isHeader) {
            idx--;
        }
        selectedIndex = idx >= 0 ? idx : Math.max(0, results.length - 1);
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            NIconButton {
                id: dateFilterButton
                visible: root.activeProvider && root.activeProvider.hasDateFilter
                icon: root.activeProvider && root.activeProvider.dateFilter !== "all" ? "filter-check" : "filter"
                tooltipText: I18n.tr("tooltips.date-filter") || "Date Filter"
                customRadius: Style.iRadiusM
                Layout.preferredWidth: searchInput.height
                Layout.preferredHeight: searchInput.height

                onClicked: {
                    dateFilterMenu.openAtItem(dateFilterButton, 0, dateFilterButton.height + Style.marginS);
                }

                NContextMenu {
                    id: dateFilterMenu
                    parent: Overlay.overlay
                    model: root.activeProvider ? root.activeProvider.availableDateFilters : []
                    onTriggered: action => {
                        if (root.activeProvider && root.activeProvider.selectDateFilter) {
                            root.activeProvider.selectDateFilter(action);
                        }
                    }
                }
            }
        ...
        Component {
            ...
            NListView {
                ...
                delegate: NBox {
                    ...
                    property bool isSelected: (!modelData.isHeader) && ((!root.ignoreMouseHover && mouseArea.containsMouse) || (index === root.selectedIndex))
                    ...
                    implicitHeight: modelData.isHeader ? Math.round(root.entryHeight * 0.45) : root.entryHeight
                    clip: true
                    ...
                    ColumnLayout {
                        id: contentLayout
                        anchors.fill: parent
                        anchors.margins: modelData.isHeader ? Style.marginXXS : (root.isCompactDensity ? Style.marginXS : Style.marginM)
                        ...
                        RowLayout {
                            ...
                            Item {
                                ...
                                Rectangle {
                                    ...
                                    color: modelData.colorHex ? modelData.colorHex : Color.mSurfaceVariant
                                    visible: (Settings.data.appLauncher.showIconBackground && !modelData.isImage) || !!modelData.colorHex
                                    border.width: modelData.colorHex ? 1 : 0
                                    border.color: modelData.colorHex ? Color.mOnSurfaceVariant : "transparent"
                                }
                                ...
                                NText {
                                    id: stringDisplay
                                    anchors.centerIn: parent
                                    visible: !!modelData.displayString || (!imagePreview.visible && !iconLoader.visible && !modelData.colorHex)
                            ...
                            ColumnLayout {
                                ...
                                Layout.leftMargin: modelData.isHeader ? Style.marginM : 0
                                spacing: 0

                                NText {
                                    text: modelData.name || "Unknown"
                                    pointSize: modelData.isHeader ? Style.fontSizeM : Style.fontSizeL
                                    font.weight: Style.fontWeightBold
                    ...
                    MouseArea {
                        ...
                        enabled: !Settings.data.appLauncher.ignoreMouseInput
                        onEntered: {
                            if (!root.ignoreMouseHover && !modelData.isHeader) {
                                root.selectedIndex = index;
                            }
                        }
                        onClicked: mouse => {
                            if (mouse.button === Qt.LeftButton && !modelData.isHeader) {
                                root.selectedIndex = index;
                                root.activate();
                                mouse.accepted = true;
                            }
