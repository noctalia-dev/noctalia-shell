...
Item {
    ...
    function getSelectedEngineKey() {
        var key = Settings.data.appLauncher
                && Settings.data.appLauncher.searchEngine ? String(
                                                                Settings.data.appLauncher.searchEngine) : ""
    ...
    function getEngineTemplate(key) {
        ...
        if (key === "custom") {
            var t = Settings.data.appLauncher
                    && Settings.data.appLauncher.searchEngineCustomTemplate ? Settings.data.appLauncher.searchEngineCustomTemplate : ""
            return t || ""
        }
    ...
    function commands() {
        return [{
                    "name": ">search",
                    "description": I18n.tr(
                                       "launcher.providers.websearch-description"),
                    ...
                    "name": ">web",
                    "description": I18n.tr(
                                       "launcher.providers.websearch-description-alt"),
    ...
    function getResults(query) {
        ...
        if (q.startsWith(">search") || q.startsWith(">web")) {
            var expression = q.indexOf(
                        " ") >= 0 ? q.substring(q.indexOf(" ")).trim() : ""
            if (!expression) {
                // Prompt entry telling user to type query
                return [{
                            "name": I18n.tr(
                                        "launcher.providers.websearch-empty"),
                            "description": I18n.tr(
                                               "launcher.providers.websearch-empty-description"),
                            "icon": iconMode === "tabler" ? "search" : "system-search",
                            "isTablerIcon": true,
                            "isImage": false,
            ...
            if (url) {
                results.push({
                                 "name": I18n.tr(
                                             "launcher.providers.websearch-search-with",
                                             {
            ...
            var wiki = buildUrlFromTemplate(getEngineTemplate("wikipedia"),
                                            expression)
            var yt = buildUrlFromTemplate(getEngineTemplate("youtube"),
                                          expression)

            if (wiki) {
                results.push({
                                 "name": I18n.tr(
                                             "launcher.providers.websearch-search-wikipedia",
                                             {
                                                 "query": expression
                                             }),
                                 "description": wiki,
            ...
            if (yt) {
                results.push({
                                 "name": I18n.tr(
                                             "launcher.providers.websearch-search-youtube",
                                             {
                                                 "query": expression
                                             }),
                                 "description": yt,
                                 "icon": iconMode
                                         === "tabler" ? "brand-youtube" : "video-x-generic",
                                 "isTablerIcon": true,
                                 "isImage": false,
                                 "provider": root,
                                 "onActivate": function () {
        ...
        var mainUrl = templateMain ? buildUrlFromTemplate(templateMain,
                                                          trimmed) : ""
        ...
        var wikiUrl = buildUrlFromTemplate(getEngineTemplate("wikipedia"),
                                           trimmed)
        if (wikiUrl && wikiUrl !== mainUrl) {
            res.push({
                         "name": I18n.tr(
                                     "launcher.providers.websearch-search-wikipedia-short",
                                     {
                                         "query": trimmed
                                     }),
                         "description": wikiUrl,
        ...
        if (ytUrl && ytUrl !== mainUrl && ytUrl !== wikiUrl) {
            res.push({
                         "name": I18n.tr(
                                     "launcher.providers.websearch-search-youtube-short",
                                     {
                                         "query": trimmed
                                     }),
                         "description": ytUrl,
    ...
    function getItemActions(item) {
        ...
        // Provide an "Open in new engine" submenu-like action not implemented here,
        // but we give a simple quick action: open engine selection settings
        return [{
                    "icon": "settings",
                    "tooltip": I18n.tr(
                                   "launcher.providers.websearch-change-engine"),
                    "action": function () {
                        ...
                        if (root.launcher) {
                            ...
                            if (typeof SettingsService !== "undefined"
                                    && SettingsService.openSection) {
                                SettingsService.openSection(
                                            "appLauncher.search")
                            } else {
                                ToastService.showNotice(
                                            I18n.tr("toast.open-settings"),
                                            I18n.tr(
                                                "launcher.providers.websearch-change-engine"))
                            }
                            root.launcher.close()
    ...
    // Initialization
    function init() {
        Logger.d("WebSearchProvider",
                 "Initialized (engine: " + getSelectedEngineKey() + ")")
    }
}
➜  ~ qmlfmt Documents/Github/Repositories/Contributing/noctalia-shell/Modules/Panels/Launcher/Providers/WebSearchProvider.qml
import QtQuick
import Quickshell
import qs.Commons
