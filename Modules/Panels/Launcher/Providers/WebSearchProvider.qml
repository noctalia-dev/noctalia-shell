...
Item {
  ...
  readonly property var engines: ({
    "google": { "name": "Google", "template": "https://www.google.com/search?q={q}" },
    "duckduckgo": { "name": "DuckDuckGo", "template": "https://duckduckgo.com/?q={q}" },
    "bing": { "name": "Bing", "template": "https://www.bing.com/search?q={q}" },
    "brave": { "name": "Brave Search", "template": "https://search.brave.com/search?q={q}" },
    "ecosia": { "name": "Ecosia", "template": "https://www.ecosia.org/search?q={q}" },
    // Quick site shortcuts
    "wikipedia": { "name": "Wikipedia", "template": "https://en.wikipedia.org/w/index.php?search={q}" },
    "youtube": { "name": "YouTube", "template": "https://www.youtube.com/results?search_query={q}" },
    // custom: use Settings.data.appLauncher.searchEngineCustomTemplate when "custom" selected
  ...
  function getResults(query) {
    ...
    if (q.startsWith(">search") || q.startsWith(">web")) {
      ...
      if (url) {
        results.push({
          "name": I18n.tr("launcher.providers.websearch-search-with", {engine: (engines[engineKey] ? engines[engineKey].name : engineKey), query: expression}),
          "description": url,
      ...
      if (wiki) {
        results.push({
          "name": I18n.tr("launcher.providers.websearch-search-wikipedia", {query: expression}),
          "description": wiki,
      ...
      if (yt) {
        results.push({
          "name": I18n.tr("launcher.providers.websearch-search-youtube", {query: expression}),
          "description": yt,
    ...
    if (mainUrl) {
      res.push({
        "name": I18n.tr("launcher.providers.websearch-search-with", {engine: (engines[engineKey] ? engines[engineKey].name : engineKey), query: trimmed}),
        "description": mainUrl,
        "icon": iconMode === "tabler" ? "search" : "system-search",
        "isTablerIcon": true,
        "isImage": false,
    ...
    if (wikiUrl && wikiUrl !== mainUrl) {
      res.push({
        "name": I18n.tr("launcher.providers.websearch-search-wikipedia-short", {query: trimmed}),
        "description": wikiUrl,
        "icon": iconMode === "tabler" ? "book" : "text-x-generic",
        "isTablerIcon": true,
        "isImage": false,
    ...
    if (ytUrl && ytUrl !== mainUrl && ytUrl !== wikiUrl) {
      res.push({
        "name": I18n.tr("launcher.providers.websearch-search-youtube-short", {query: trimmed}),
        "description": ytUrl,
        "icon": iconMode === "tabler" ? "brand-youtube" : "video-x-generic",
        "isTablerIcon": true,
        "isImage": false,
