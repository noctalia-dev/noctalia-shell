...
Item {
    ...
    function init() {
        Logger.i("WebSearchPlugin",
                 "Initialized (default engine: " + defaultEngine + ")")
    }
    ...
    function formateSearchEntry(engine, query) {
        var q = query.trim()
        var title = q === "" ? I18.tr("plugins.search.use-engine").replace(
                                   "%engine%", engine.name) : I18.tr(
                                   "plugins.search.search-with").replace(
                                   "%engine%",
                                   engine.name).replace("%query%", q)
        var desc = q === "" ? I18n.tr("plugins.search.set-default") : I18n.tr(
                                  "plugins.search.open-in-browser")
