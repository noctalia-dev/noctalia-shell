...
import qs.Services.Location // Added this import
import qs.Widgets
...
ColumnLayout {
  ...
  NToggle {
	...
	enabled: Settings.data.colorSchemes.schedulingMode === "off" // Re-added this line
	onToggled: checked => {
				 // This toggle is only enabled when schedulingMode is "off",
				 // so any change here is a manual change.
				 Settings.data.colorSchemes.darkMode = checked
				 DarkModeService.cache.manualDarkMode = checked
				 // Re-added this line
				 // Remember this manual choice
				 root.cacheVersion++
				 // Force UI update for dark/light variants
			   }
  }
  ...
  NComboBox {
	...
	onSelected: key => {
				  const oldMode = Settings.data.colorSchemes.schedulingMode
				  // Re-added this line

				  // If we are switching AWAY from "off" mode to "manual" or "location"
				  if (oldMode === "off" && key !== "off") {
					// Re-added this line
					// Remember the current state before the schedule takes over
					DarkModeService.cache.manualDarkMode = Settings.data.colorSchemes.darkMode
					// Re-added this line
				  }

				  Settings.data.colorSchemes.schedulingMode = key
				  // Update the scheduling mode setting

				  // Activate/Deactivate the appropriate mode in DarkModeService // Re-added this block
				  if (key === "manual") {
					DarkModeService.activateManualMode()
				  } else if (key === "location") {
					DarkModeService.activateLocationMode()
				  } else {
					// key === "off"
					DarkModeService.deactivateScheduling()
					// Restore the user's last manual setting from the cache
					Settings.data.colorSchemes.darkMode = DarkModeService.cache.manualDarkMode
				  }

				  // Defer generation to ensure property changes have propagated // Re-added this line
				  Qt.callLater(AppThemeService.generate)
				}
  }
