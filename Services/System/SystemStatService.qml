...
Singleton {
  ...
  FileView {
    ...
    onLoaded: {
      ...
      if (gpu.type === "amd") {
        const isIgpu = GpuDatabase.amdIgpuDeviceIds.includes(deviceId);
        if (GpuDatabase.amdGpuNames[deviceId]) {
          gpu.deviceName = `AMD ${GpuDatabase.amdGpuNames[deviceId]}`;
        }
        ...
        // Intel iGPU
        if (GpuDatabase.intelGpuNames[deviceId]) {
          gpu.deviceName = `Intel ${GpuDatabase.intelGpuNames[deviceId]}`;
        } else {
  ...
  function isGpuDiscrete(gpu) {
    if (gpu.type === "nvidia") {
      ...
      return !GpuDatabase.amdIgpuDeviceIds.includes(gpu.deviceId);
    }
    return false;
  }
  ...
  function getGpuDisplayName(gpu) {
    ...
    if (gpu.type === "nvidia") {
      ...
      if (gpu.deviceId && GpuDatabase.amdGpuNames[gpu.deviceId]) {
        const name = GpuDatabase.amdGpuNames[gpu.deviceId];
