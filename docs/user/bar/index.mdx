...
| `monitor_focus_behavior` | string | `ignored` | Hide this widget depending on the monitor's focus state: `ignored` always shows it (default), `focused` shows it only on the currently focused monitor, and `unfocused` shows it only on non-focused monitors. |
