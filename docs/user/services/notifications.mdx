...
- Notification summaries are always plain text. Noctalia does not advertise body-markup support, so it strips the
  standard notification body tags (`b`, `i`, `u`, `a`, `img`, `br`) while preserving malformed or unsupported
  angle-bracket text literally. A `br` tag becomes a line break.
