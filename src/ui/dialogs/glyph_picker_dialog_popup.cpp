...
void GlyphPickerDialogPopup::requestFontLayout() {
  if (m_surface != nullptr) {
    m_surface->requestLayout();
  }
}

void GlyphPickerDialogPopup::requestThemeRedraw() {
  if (m_surface != nullptr) {
    m_surface->requestRedraw();
  }
}

wl_surface* GlyphPickerDialogPopup::wlSurface() const noexcept {
...
void GlyphPickerDialogPopup::cancel() {
  if (m_surface == nullptr) {
    return;
  }
  destroyPopup();
...
void GlyphPickerDialogPopup::prepareFrame(bool needsUpdate, bool needsLayout) {
...
void GlyphPickerDialogPopup::buildScene(std::uint32_t width, std::uint32_t height) {
...
void GlyphPickerDialogPopup::layoutScene(float width, float height) {
...
void GlyphPickerDialogPopup::destroyPopup() {
...
bool GlyphPickerDialogPopup::mapPointerEvent(const PointerEvent& event, float& localX, float& localY) const noexcept {
  if (m_surface == nullptr) {
    return false;
  }
...
void GlyphPickerDialogPopup::syncPointerStateFromCurrentPosition() {
...
bool GlyphPickerDialogPopup::ownsSurface(wl_surface* surface) const noexcept {
  return m_surface != nullptr && surface != nullptr && surface == m_surface->wlSurface();
}

wl_surface* GlyphPickerDialogPopup::resolveEventSurface(const PointerEvent& event) const noexcept {
...
std::optional<LayerPopupParentContext> GlyphPickerDialogPopup::resolveParentContext() const {
...
float GlyphPickerDialogPopup::uiScale() const {
  if (m_config == nullptr) {
    return 1.0f;
  }
