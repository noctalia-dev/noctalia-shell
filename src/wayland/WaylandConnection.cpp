...
void WaylandConnection::activateWorkspace(const std::string& id) {
    m_workspaces_handler.activate(id);
}

std::vector<Workspace> WaylandConnection::workspaces() const {
    return m_workspaces_handler.all();
}
...
bool WaylandConnection::hasExtWorkspaceManager() const noexcept {
    return m_hasExtWorkspaceGlobal;
}
...
void WaylandConnection::handleGlobal(void* data, wl_registry* registry,
...
void WaylandConnection::handleGlobalRemove(void* data, wl_registry* /*registry*/,
                                           std::uint32_t name) {
    auto* self = static_cast<WaylandConnection*>(data);
...
void WaylandConnection::bindGlobal(wl_registry* registry, std::uint32_t name,
