#include "wayland/WaylandWorkspaces.hpp"
...
void managerWorkspaceGroup(void* data, ext_workspace_manager_v1* /*manager*/,
                           ext_workspace_group_handle_v1* group) {
    static_cast<WaylandWorkspaces*>(data)->onGroupCreated(group);
}

void managerWorkspace(void* data, ext_workspace_manager_v1* /*manager*/,
...
void WaylandWorkspaces::onGroupWorkspaceEnter(ext_workspace_group_handle_v1* group, ext_workspace_handle_v1* workspace) {
    for (auto& g : m_groups) {
        if (g.handle == group) {
...
void WaylandWorkspaces::onGroupWorkspaceLeave(ext_workspace_group_handle_v1* group, ext_workspace_handle_v1* workspace) {
    for (auto& g : m_groups) {
        if (g.handle == group) {
